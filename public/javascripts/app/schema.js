...
var Type = function(name){
    this.properties = {};
    this.type = schema.types[name];
    this.uri = this.type["url"];
    this.ancestors = this.type["ancestors"];
    for(i=0;i<this.ancestors.length;i++)
    {
        var ancestor = schema.types[this.ancestors[i]]["properties"];
        //console.log(ancestor);
        for(j=0;j<ancestor.length;j++)
        {
        if(this.properties[ancestor[j]]==undefined)
            this.properties[ancestor[j]]=null;
        }
    }
};
